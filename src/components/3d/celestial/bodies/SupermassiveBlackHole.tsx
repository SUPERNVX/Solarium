...
const BLACK_HOLE_RADIUS = 2.5; // Larger
const DISK_INNER_RADIUS = BLACK_HOLE_RADIUS + 0.5;
const DISK_OUTER_RADIUS = 15.0; // Much wider
const DISK_TILT_ANGLE = Math.PI / 3.0;

export default function SupermassiveBlackHole() {
        ...
        return new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    fresnel = pow(fresnel, 2.0); vec3 glowColor = vec3(1.0, 0.2, 0.2); // Reddish glow
    ...
    const diskMaterial = useMemo(() => {
        return new THREE.ShaderMaterial({
            uniforms: {
                ...
                uColorMid1: { value: new THREE.Color(0xff5500) }, uColorMid2: { value: new THREE.Color(0xaa1100) },
                uColorOuter: { value: new THREE.Color(0x330000) }, uNoiseScale: { value: 2.0 },
                uFlowSpeed: { value: 0.15 }, uDensity: { value: 2.0 }
            },
            vertexShader: `
            ...
            fragmentShader: `
                ...
                void main() {
                    float normalizedRadius = smoothstep(3.0, 15.0, vRadius);
                    float timeOffset = uTime * uFlowSpeed * (2.0 / (vRadius * 0.2 + 1.0));
                    vec2 noiseUv = vec2(vAngle * 2.0 + timeOffset, vRadius * 0.4);
                    float noiseVal1 = snoise(vec3(noiseUv * uNoiseScale, uTime * 0.1));
                    float noiseVal2 = snoise(vec3(noiseUv * uNoiseScale * 2.0 + 0.8, uTime * 0.15));
                    float noiseVal = (noiseVal1 * 0.6 + noiseVal2 * 0.4); noiseVal = (noiseVal + 1.0) * 0.5;
                    vec3 color = mix(uColorOuter, uColorMid2, smoothstep(0.0, 0.3, normalizedRadius));
                    color = mix(color, uColorMid1, smoothstep(0.2, 0.6, normalizedRadius));
                    color = mix(color, uColorHot, smoothstep(0.5, 0.9, normalizedRadius));
                    float brightness = pow(1.0 - normalizedRadius, 1.5) * 4.0 + 0.5;
                    brightness *= (0.5 + noiseVal * 2.0);
                    float radialGrid = 1.0 - (sin(vRadius * 2.0 - uTime * 0.3) * 0.5 + 0.5);
                    radialGrid = pow(radialGrid, 4.0); float angleGrid = 1.0 - (sin(vAngle * 30.0) * 0.5 + 0.5);
                    ...
                    alpha *= smoothstep(0.0, 0.1, normalizedRadius); alpha *= (1.0 - smoothstep(0.9, 1.0, normalizedRadius));
    ...
    useFrame(({ clock, camera: currentCamera }) => {
        ...
        if (diskRef.current) {
            ...
            diskRef.current.rotation.z += delta * 0.8; // Faster due to massive scale
        }
